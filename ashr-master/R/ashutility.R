...
plot_diagnostic = function (x, plot.it = TRUE, 
  ...
  if (plot.it) {
    ...
    graphics::plot((1:n)/(n + 1), sort(cdfhat[!na.ind]), 
                   xlim = c(0, 1), ylim = c(0, 1), xlab = "Theoretical U[0,1] Quantile", 
                   ylab = expression(paste("Estimated ", hat(H)[(j)])), main = "(a): Q-Q Plot", 
                   pch = pch, cex = cex)
    abline(0, 1, lty = 2, col = "red")
    cat("Press [enter] to see next plot")
    line <- readline()
    graphics::plot(diff, cex = cex, pch = pch, ylim = range(diff, 
                                                            upper, lower, 0), xlab = "Index j", ylab = expression(paste("Estimated ", hat(H)[(j)] - j/(p+1))), main = "(b): De-meaned Order Statistics")
    ...
    graphics::hist(cdfhat[!na.ind], probability = TRUE, breaks = breaks, 
                   xlab = expression(hat(H)[j]), main = paste("(c): Histogram; K-S p-value = ", p.ks.unif))
    ...
    if (plot.hist) {
      betahat <- x$data$x
      if (missing(xmin)) {
        xmin = min(betahat, na.rm = TRUE)
      }
      if (missing(xmax)) {
        xmax = max(betahat, na.rm = TRUE)
      }
      xgrid.length = 1000
      ...
      if (sebetahat.same) {
        plot.data$x = xgrid
        plot.data$s = rep(mean(sebetahat, na.rm = TRUE), 
                          xgrid.length)
      ...
      hist.betahat = graphics::hist(betahat[!na.ind], breaks = breaks, 
                                    plot = FALSE)
      graphics::hist(betahat[!na.ind], probability = TRUE, 
                     breaks = breaks, ylim = c(0, max(c(fhat, hist.betahat$density))), 
                     xlab = expression(X[j]), main = expression(paste("(d): Histogram of ", 
                                                                           X[j])))
      lines(xgrid, fhat, col = "blue")
      legend("topleft", lty = 1, col = "blue", expression(hat(h)))
    }
  }
  invisible(cdfhat)
}
